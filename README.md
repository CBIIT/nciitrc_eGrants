# eGrants Federal Development Project

Welcome to the eGrants Development Project. This project supports the eGrants Mission Critical web application as well as the full stack of the project. 
## Table of Contents
...
# Coding
The eGrants application was developed many years ago and has been constantly upgraded to meet client requirements. The original code base is over 10 years old. The .NET Framework has been updated to .NET 4.8 but this does not mean that the MVC design is following the correct design patters.

Entity Framework is not part of this project. All calls to the DB are done with inline SQL and called to Stored Prodcedures.

There is much that can be improved on the application given the correct resources.
...
[Resharper](https://www.jetbrains.com/resharper/) is a tool to analyze C# source code and offer alternate solutions as well as analyze Cyclomati Complexity and enforce modern standards.
### How to use eGrants Locally

* Clone a copy from the CBIIt/nciitrc_eGrants repository. 
    ** You will need access to this through NIH's single sign on and authorization from the lead developer. 
* Everything in the project is configured to run and ready to go. You do not need IIS configured locally and instead IIS Express will suit the needs local development just fine.

## Web.Config  
* Config file transformations have been created to aid in the bebugging of the application and Jenkins deployment.
