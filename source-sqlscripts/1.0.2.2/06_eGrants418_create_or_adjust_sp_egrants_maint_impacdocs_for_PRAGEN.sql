USE [EIM]
GO
/****** Object:  StoredProcedure [dbo].[sp_egrants_maint_impacdocs_PRAGEN]    Script Date: 12/8/2023 4:24:15 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

CREATE OR ALTER PROCEDURE [dbo].[sp_egrants_maint_impacdocs_PRAGEN] 
AS
...
SET @PROC_NAME = 'sp_egrants_maint_impacdocs_PRAGEN'
SET @RUN_DATETIME = GETDATE()

BEGIN
print 'BEGIN sp_egrants_maint_impacdocs_PRAGEN = @ '+ cast(getdate() as varchar)

TRUNCATE TABLE dbo.impp_PriorApproval

INSERT dbo.impp_PriorApproval(APPL_ID,DOCUMENT_DATE,keyId,Request_id)
...
SELECT @CATEGORYID=category_id from categories where impac_doc_type_code='PRAGEN' 
...
select zz.appl_id,@CATEGORYID,zz.document_date,@impacID,getdate(),'pdf',NULL,dbo.fn_grant_profile_id(ee.grant_id),zz.Request_id,'Other'
from impp_PriorApproval zz, vw_appls ee 
where zz.appl_id=ee.appl_id and ee.admin_phs_org_code='CA'
and zz.Request_id not in 
--4/24/2019:Imran:If ImpacII has not created any PRAGEN there will be no nga_rpt_seq_num therefore create one, If created and got anotated then do not add another
...
print 'NEW Prior-approval (singular) INSERTED =' + cast(@@ROWCOUNT as varchar)
EXEC DBO.Record_egrants_docload_log  @PROC_NAME,@RUN_DATETIME,@CATEGORYID,'PRAGEN',@IMPAC_DOWNLOAD_CNT,@EGRANTS_UPLOAD_CNT,null
print 'END sp_egrants_maint_impacdocs_PRAGEN = @ '+ cast(getdate() as varchar)
END
