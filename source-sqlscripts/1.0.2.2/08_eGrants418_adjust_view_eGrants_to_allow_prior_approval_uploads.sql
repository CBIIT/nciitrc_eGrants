...
/****** Object:  View [dbo].[egrants]    Script Date: 1/24/2024 10:54:54 AM ******/
SET ANSI_NULLS ON
GO
...
CREATE OR ALTER           VIEW [dbo].[egrants]
AS
...
SELECT     dbo.profiles.profile AS ic, dbo.grants.grant_id, dbo.grants.admin_phs_org_code, dbo.grants.serial_num, 
                      ...
                      WHEN (dbo.documents.category_id in (
						 select (category_id) from categories where (category_name = 'Application File' OR category_name = 'Data Management and Sharing (DMS) Plan' OR category_name = 'Prior Approval' )
					  )) then 'y'
