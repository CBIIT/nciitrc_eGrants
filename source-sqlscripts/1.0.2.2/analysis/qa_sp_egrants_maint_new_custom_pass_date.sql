USE [EIM]
GO
/****** Object:  StoredProcedure [dbo].[sp_egrants_maint_new_custom_pass_date.sql]    Script Date: 12/20/2023 3:17:15 PM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER OFF
GO

ALTER    PROCEDURE  [dbo].[sp_egrants_maint_new_custom_pass_date.sql] AS
...
begin catch 
 --SELECT ERROR_MESSAGE() AS ERROR
	print(' step3 - Error updating appls '  + ERROR_MESSAGE()) -- Changed by Madhu
 end catch
 */
DROP TABLE IF EXISTS #appl_ids_to_be_updated

SELECT IDENTITY(INT, 1, 1) AS ID, *
INTO #appl_ids_to_be_updated
from appls
where new_appl_id is not null 
and new_appl_id not in(select appl_id from appls)

Declare @min_id int = 0, @max_id int = 0, @loopId int = 0, @tempTableRecordCount int = 0, @RecordsUpdated int = 0
Declare @currAppl_id varchar(20) = null

select @tempTableRecordCount = count(*) from #appl_ids_to_be_updated
print 'Step5 - @tempTableRecordCount  ' + cast(@tempTableRecordCount as varchar)

IF (@tempTableRecordCount > 0)
BEGIN
	select @min_id = min(id), @max_id = max(id) from #appl_ids_to_be_updated
	print 'Processing #appl_ids_to_be_updated '
	print '@min_id  = ' + cast(@min_id as varchar) + ' ---- @max_id = ' + cast(@max_id as varchar)
	set @loopId = @min_id 
	WHILE(@loopId <= @max_id)
		BEGIN
			BEGIN TRY 
				...
				select  @currAppl_id = appl_id from #appl_ids_to_be_updated where id = @loopId
				UPDATE appls SET appl_id=new_appl_id WHERE appl_id = @currAppl_id
				print '---- Updating appls table where appl_id = ' +  cast(@currAppl_id as varchar) 
			END TRY 
			BEGIN  CATCH 
				print 'Step5 - Error Updating appls for Appl_id: ' + CAST(@currAppl_id AS VARCHAR) + ' - ' + ERROR_MESSAGE();  

				-- if records fail to be updated, the count needs to change
				set @RecordsUpdated = @RecordsUpdated - 1 

			END CATCH
			set @loopId = @loopId + 1
		END
print 'done with Step5 - Updated ' + cast(@RecordsUpdated as varchar) + ' appl_id records.'
END
ELSE
BEGIN
	print 'done with Step5 --- Updated no appl_id records. @tempTableRecordCount being zero'
END
...
--Inactive a user if his auth period has expired.

update people set Active=0
where end_date is not null and end_date < getdate() and active=1
	print 'done with step22'
...
print 'Completed sp_egrants_maint_new_custom_pass_date.sql'
