USE [EIM]
GO
/****** Object:  StoredProcedure [dbo].[getPlaceHolder_new]    Script Date: 9/9/2024 11:05:55 AM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

ALTER PROCEDURE [dbo].[getPlaceHolder_new]
...
@emailto varchar(200),@emailcc varchar(200),@emailTempID int,@pos varchar(50)
	...
	SELECT @SrlNumber = serial_num, 
        @SupportYear=support_year, 
        @SuffixCode = suffix_code, 
        @Parent_fgn = full_grant_num, 
        @full_grant_num = full_grant_num 
    FROM [EIM].[dbo].[vw_appls] 
    WHERE appl_id=@PARENTAPPLID

	INSERT [EIM].[dbo].[Imm_Capture_Error] (Error,Detail,timestmp)
...
-- PRINT @body		
	...
	IF (@Catname='eRA Notification')
        -- RB 06/14/2022 - Removed the 'AND NOT EXISTS' check on this record so we send multiple notifications for a single Grant in a single year
        -- AND NOT EXISTS (select * from dbo.adsup_notification where Replace(subjectLine,' ','')=Replace(@Sub,' ','') and disabled_date is null) 
    BEGIN
