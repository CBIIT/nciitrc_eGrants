USE [EIM]
GO
/****** Object:  StoredProcedure [dbo].[sp_web_egrants_doc_url_restore]    Script Date: 9/26/2023 8:03:47 AM ******/
SET ANSI_NULLS OFF
GO
SET QUOTED_IDENTIFIER OFF
GO

ALTER     PROCEDURE [dbo].[sp_web_egrants_doc_url_restore]
...
--print(concat('category: ', @Category));

SET @url=
CASE 
WHEN (@Category='Application File' or @Category='Data Management and Sharing (DMS) Plan') and @CreatedBy in ('impac', 'efile') 
...
--THEN 'data/funded2/nci/main1/' + convert(varchar,@DocID) + '.' + @FType -- Commented by Madhu on 09/13/2022
THEN dbo.fn_get_env_url('data_funded2_main1') + convert(varchar,@DocID) + '.' + @FType -- Added by Madhu 09/13/2022

WHEN @DocID>=24000000 and @CreatedBy not in ('impac', 'efile') and @supp_backup_url is null
--THEN 'data/funded2/nci/main/' + convert(varchar,@DocID) + '.' + @FType -- Commented by Madhu on 09/13/2022
THEN dbo.fn_get_env_url('data_funded2_main') + convert(varchar,@DocID) + '.' + @FType -- Added by Madhu 09/13/2022
...
-- THEN 'data/funded2/nci/main1/' + convert(varchar,@DocID) + '.txt' -- Commented by Madhu on 09/13/2022
THEN dbo.fn_get_env_url('data_funded2_main1') + convert(varchar,@DocID) + '.txt' -- Added by Madhu 09/13/2022

ELSE
'cannot restore'
END

--print(concat('outcome: ', @url))
