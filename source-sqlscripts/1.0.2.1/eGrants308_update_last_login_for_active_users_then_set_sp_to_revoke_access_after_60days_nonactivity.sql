...
PRINT '----------- Adjusting [dbo].[sp_egrants_maint_check_expired_people] to 60 day expiration ----------'
...
AS
	...
	SET @sql='select * from people WHERE last_login_date < DateAdd(day, -60, GETDATE()) AND Active=1'
	EXEC(@sql)
	PRINT(@sql)

	UPDATE [dbo].[people]
	SET active = 0
	WHERE last_login_date is not null AND last_login_date < DateAdd(day, -60, GETDATE()) AND active=1;