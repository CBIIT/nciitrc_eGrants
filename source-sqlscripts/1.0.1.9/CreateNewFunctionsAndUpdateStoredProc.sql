...
------------------------------------- Create function fn_get_org_name_by_applid_specific_year ---------------------------------------------

USE [EIM]
GO

/****** Object:  UserDefinedFunction [dbo].[fn_get_org_name_by_applid_specific_year]    Script Date: 5/4/2023 2:03:26 PM ******/
SET ANSI_NULLS OFF
GO
...
set @pi_name=(
	select org_name from vw_appls where appl_id=@appl_id
)
...
------------------------------------- Create Function fn_get_pi_name_by_applid_specific_year --------------------------------------
USE [EIM]
GO

/****** Object:  UserDefinedFunction [dbo].[fn_get_pi_name_by_applid_specific_year]    Script Date: 5/4/2023 1:59:14 PM ******/
SET ANSI_NULLS OFF
GO
...
declare @pi_name  varchar(92)
set @pi_name=(
	select (a.first_name + ' ' + a.last_name) as pi_name 
    from vw_appls a
    Inner join vw_grants g on g.grant_id = a.grant_id
    LEFT jOIN vw_appls s on s.appl_id = a.appl_id and a.grant_id = g.grant_id  and s.support_year = a.support_year and s.appl_type_code in  (1,2,5,7,9) and a.suffix_code = null
    --Left JOIN vw_appls s on s.grant_id = a.grant_id  --(3,4,6,8)
...
------------------------------------- Create Function fn_get_pi_email_by_applid_specific_year -----------------------------------
USE [EIM]
GO

/****** Object:  UserDefinedFunction [dbo].[fn_get_pi_email_by_applid_specific_year]    Script Date: 5/4/2023 2:00:10 PM ******/
SET ANSI_NULLS OFF
GO
...
declare @pi_name  varchar(92)
set @pi_name=(
	select PI_EMAIL_ADDR from appls where appl_id=@appl_id
)
...
------------------------------------- Create Function fn_get_project_name_by_applid_specific_year ---------------------------------------
USE [EIM]
GO

/****** Object:  UserDefinedFunction [dbo].[fn_get_project_name_by_applid_specific_year]    Script Date: 5/4/2023 2:01:11 PM ******/
SET ANSI_NULLS OFF
GO
...
declare @pi_name  varchar(92)
set @pi_name=(
	select project_title from vw_appls where appl_id=@appl_id
)
...
------------------------------------- Create Function fn_get_institution1_by_applid_specific_year ----------------------------------
USE [EIM]
GO

/****** Object:  UserDefinedFunction [dbo].[fn_get_institution1_by_applid_specific_year]    Script Date: 5/4/2023 2:02:04 PM ******/
SET ANSI_NULLS OFF
GO
...
declare @inst1  varchar(92)
set @inst1=(
	select 
	CASE WHEN EXISTS(
		Select document_id from vw_org_document od
		inner join Org_Master om on od.org_id = om.org_id where category_name = 'Follow-Up' 
			and CONVERT(date,end_date_showFlag) >= CONVERT(date,GETDATE()) 
			and om.org_name = a1.org_name group by od.document_id
		) THEN 1 ELSE 0 END
	from vw_appls a1
	where a1.appl_id = @appl_id
)
...
------------------------------------- Create Function fn_get_institution2_by_applid_specific_year ----------------------------
USE [EIM]
GO

/****** Object:  UserDefinedFunction [dbo].[fn_get_institution2_by_applid_specific_year]    Script Date: 5/4/2023 2:02:41 PM ******/
SET ANSI_NULLS OFF
GO
...
set @inst1=(
	select 
	CASE WHEN EXISTS(Select org_id from vw_org_document vod where vod.Org_name = a1.org_name) THEN 1 ELSE 0 END
	from vw_appls a1
	where a1.appl_id = @appl_id
)
...
------------------------------------- Create Function fn_get_full_grant_num_by_applid_specific_year ----------------------------

USE [EIM]
GO

/****** Object:  UserDefinedFunction [dbo].[fn_get_full_grant_num_by_applid_specific_year]    Script Date: 5/10/2023 4:55:52 PM ******/
SET ANSI_NULLS OFF
GO
...
declare @full_grant_num  varchar(20)
set @full_grant_num=(
	  select full_grant_num from vw_appls where appl_id = @appl_id
)
...
------------------------------------- Update sp_web_egrants ----------------------------------

USE [EIM]
GO

/****** Object:  StoredProcedure [dbo].[sp_web_egrants]    Script Date: 5/10/2023 5:11:02 PM ******/
SET ANSI_NULLS ON
GO
