    ...
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            try
            {
                ...
                if (filesMoved > 0)
                {
                    WriteLog($"{filesMoved} file(s) moved from network share to {docSrcPath}", null, DateTime.Now, logDir);
                    CommonUtilities.ShowDiagnosticIfVerbose($"{filesMoved} file(s) moved from network share.", verbose);
                }
                else
                {
                    WriteLog("No files found on network share to move.", null, DateTime.Now, logDir);
                ...
                if (xmlFileCount == 0)
                {
                    WriteLog("No files to process in DocSrcPath. Exiting.", null, DateTime.Now, logDir);
                    CommonUtilities.ShowDiagnosticIfVerbose("No files to process in DocSrcPath. Exiting.", verbose);
                    return;
                }

                WriteLog($"Found {xmlFileCount} XML file(s) in DocSrcPath to process.", null, DateTime.Now, logDir);
