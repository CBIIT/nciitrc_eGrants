...
# Copies all files from network share to local SUPP directory.
...
try {
    ...
    if (-not $files -or $files.Count -eq 0) {
        Write-Log "No files found. Nothing to copy."
        exit 0
    }
    ...
    foreach ($file in $files) {
        ...
        Write-Log "Copied: '$($file.FullName)' -> '$targetPath'"
    }

    Write-Log "Task completed successfully. Files copied: $($files.Count)"
    exit 0
}
catch {
