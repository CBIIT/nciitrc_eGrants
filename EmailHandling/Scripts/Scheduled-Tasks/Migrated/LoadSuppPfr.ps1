...
$taskDescription = "Runs LoadSuppPfr.bat Monday through Friday at hours 7-19, on minutes 3,18,58 of each hour."

$execute = "C:\eGrants\apps\LoadSuppPfr\LoadSuppPfr.bat"
...
try {
    Write-Diag "===== START LoadSuppPfr scheduled task setup ====="
    ...
    if (-not (Test-Path $execute)) {
        ...
        Write-Diag "Searching extracted folder for LoadSuppPfr.bat..."

        $foundBatFiles = @(Get-ChildItem -Path $extractTemp -Recurse -File -Filter "LoadSuppPfr.bat" -ErrorAction SilentlyContinue)

        if (@($foundBatFiles).Count -gt 0) {
            Write-DiagWarn "Found LoadSuppPfr.bat in unexpected location(s):"
            $foundBatFiles | ForEach-Object {
                Write-Host "         $($_.FullName)"
            }
        }
        else {
            Write-DiagWarn "No LoadSuppPfr.bat file was found anywhere under: $extractTemp"
        }
    ...
    foreach ($hour in $hours) {
        foreach ($minute in $minutes) {
            ...
            Write-Diag "Creating trigger for hour $hour minute $minute with StartBoundary $startBoundary"
