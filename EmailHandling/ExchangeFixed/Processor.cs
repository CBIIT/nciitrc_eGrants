...
namespace ExchangeFixed
{
    public class Processor
    {
            ...
            while (itemToProcess > 0)
            {
                try
                {
                    ...
                    if (item != null)
                    {
                        ...
                        if (!string.IsNullOrEmpty(p.GrantNumber) || !string.IsNullOrEmpty(p.ApplId))
                        {
                ...
                {
                    CommonUtilities.WriteLog(8, $"Error item {itemToProcess}", ex.Message, DateTime.Now);
                }
                itemToProcess--;
            }
            con.Close();
        ...
        {
            string applId = p.ApplId ?? GetApplId(RemoveSpecialChars(p.GrantNumber ?? item.Subject), con);
        ...
        private string GetApplId(string text, SqlConnection con)
        {
            try
            {
                using (var cmd = new SqlCommand($"SELECT dbo.Imm_fn_applid_match('{text}')", con))
        ...
        {
            public string GrantNumber, Category, ApplId, SubCategory, Extract;
        }
    }
}
