...
namespace EmailTests.CommonUtilitiesProject
{
    /// <summary>
    ...
    public class CommonUtilitiesIntegrationTests
    {
        ...
        public void Setup()
        {
            _testLogDir = Path.Combine(Path.GetTempPath(), "EmailTestsLogs_" + Guid.NewGuid().ToString());
         Directory.CreateDirectory(_testLogDir);
          CommonUtilties.CommonUtilities.LogDir = _testLogDir;

      // Set up environment variables for local testing
       // This allows tests to run without requiring system-level environment variables
          CommonUtilties.CommonUtilities.SetLocalTestEnvironmentVariables("egrantsuser", "{DB_PASSWORD}");
        }
...
[TestMethod]
        public void GetConfigVal_WithEnvironmentVariables_ExpandsVariables()
   {
    ...
    // Create a temporary config.csv with environment variable placeholders
 ...
 // Assert
     Assert.IsTrue(result.Contains("egrantsuser"), "Should expand EGRANTS_DB_USER");
           Assert.IsTrue(result.Contains("{DB_PASSWORD}"), "Should expand EGRANTS_DB_PASSWORD");
