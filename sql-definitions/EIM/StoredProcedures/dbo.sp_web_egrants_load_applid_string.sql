...
CREATE PROCEDURE [dbo].[sp_web_egrants_load_applid_string]
...
select appl_id from egrants where grant_id='+convert(varchar, @grant_id)+') and support_year in(
select distinct top '+ convert(varchar,@years)+' support_year from vw_appls 
where grant_id = '+convert(varchar,@grant_id) +' and doc_count<>0  order by support_year desc)'
END
