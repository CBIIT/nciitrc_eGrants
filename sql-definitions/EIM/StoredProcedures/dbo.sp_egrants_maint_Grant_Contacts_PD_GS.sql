...
INSERT INTO [dbo].GM_ACTION_QUEUE_VW (APPL_ID, APPL_TYPE_CODE, SERIAL_NUM, SUPPORT_YEAR, ACTION_FY, SUFFIX_CODE, RESP_SPEC_FULL_NAME_CODE, RESP_SPEC_EMAIL_ADDRESS, PD_EMAIL_ADDRESS, PD_FULL_NAME, ACTION_TYPE, ADMIN_PHS_ORG_CODE, RESP_SPEC_NPN_ID)  
SELECT APPL_ID, APPL_TYPE_CODE, SERIAL_NUM, SUPPORT_YEAR, ACTION_FY, SUFFIX_CODE, RESP_SPEC_FULL_NAME_CODE, RESP_SPEC_EMAIL_ADDRESS, PD_EMAIL_ADDRESS, PD_FULL_NAME, ACTION_TYPE, ADMIN_PHS_ORG_CODE, RESP_SPEC_NPN_ID
from openquery(CIIP,'SELECT APPL_ID, APPL_TYPE_CODE, SERIAL_NUM, SUPPORT_YEAR, ACTION_FY, SUFFIX_CODE, RESP_SPEC_FULL_NAME_CODE, RESP_SPEC_EMAIL_ADDRESS, PD_EMAIL_ADDRESS, PD_FULL_NAME, ACTION_TYPE, ADMIN_PHS_ORG_CODE, RESP_SPEC_NPN_ID 
FROM GM_ACTION_QUEUE_VW g
WHERE (SUFFIX_CODE IS NULL OR SUFFIX_CODE LIKE ''%A%'') AND ACTION_TYPE NOT LIKE ''REVISION'' ')
...
SET [SUFFIX_CODE] = GAQV.[SUFFIX_CODE], 
	...
	ON GC.[SERIAL_NUM] = GAQV.[SERIAL_NUM] AND GC.ADMIN_PHS_ORG_CODE = GAQV.ADMIN_PHS_ORG_CODE  --- Also include IC in matching data. 

INSERT INTO [dbo].Grant_Contacts_PD_GS (APPL_ID, APPL_TYPE_CODE, SERIAL_NUM, SUPPORT_YEAR, ACTION_FY, SUFFIX_CODE, RESP_SPEC_FULL_NAME_CODE, RESP_SPEC_EMAIL_ADDRESS, PD_EMAIL_ADDRESS, PD_FULL_NAME, ADMIN_PHS_ORG_CODE, RESP_SPEC_NPN_ID)  
SELECT GAQV.APPL_ID, GAQV.APPL_TYPE_CODE, GAQV.SERIAL_NUM, GAQV.SUPPORT_YEAR, GAQV.ACTION_FY, GAQV.SUFFIX_CODE, GAQV.RESP_SPEC_FULL_NAME_CODE, GAQV.RESP_SPEC_EMAIL_ADDRESS, GAQV.PD_EMAIL_ADDRESS, GAQV.PD_FULL_NAME, GAQV.ADMIN_PHS_ORG_CODE, GAQV.RESP_SPEC_NPN_ID
	from GM_ACTION_QUEUE_VW GAQV LEFT JOIN Grant_Contacts_PD_GS GC  
	ON GC.[SERIAL_NUM] = GAQV.[SERIAL_NUM] AND GC.ADMIN_PHS_ORG_CODE = GAQV.ADMIN_PHS_ORG_CODE
	where GC.SERIAL_NUM is null
...
where LTRIM(RTRIM(PD_FULL_NAME)) = ','
	...
	FROM Grant_Contacts_PD_GS contact inner join #InactivePDs ipd on contact.PD_EMAIL_ADDRESS = ipd.PD_EMAIL_ADDRESS COLLATE DATABASE_DEFAULT
