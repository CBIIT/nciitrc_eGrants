...
CREATE PROCEDURE [dbo].[sp_egrants_mail_old2]
...
SET @count=(SELECT max(mail_id) FROM #emails)
IF @count>=1
BEGIN
...
(grantnumber varchar(20), applid int, category varchar(50), uploadid varchar(20), alias varchar(100), name varchar(100), date smalldatetime, uid varchar(20), file_type varchar(5), uploadid varchar(20)) xml
...
BEGIN
	...
	EXEC sp_rmc_egrants_doc_transaction @document_id, @operator,'create', 'created by email'
	SET @count=@count -1
